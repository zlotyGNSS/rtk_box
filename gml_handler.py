...
class MapInspector:
    def __init__(self, gml_path):
        try:
            self.gdf = gpd.read_file(gml_path, engine='pyogrio')
    ...
    def analyze_position(self, x, y):
        if self.gdf is None: return "Brak Mapy", 999.0
        p = Point(x, y)
        mask = self.gdf.contains(p)
        dzialka = self.gdf[mask][self.id_col].values[0] if mask.any() else "POZA"
