...
def handle_button():
    ...
    if current_data['q_txt'] != "FIXED":
        print(f"\n[BLOKADA] Brak FIXED! PDOP: {current_data['pdop']:.1f}"); buzzer.beep(0.3, 0.1, 1); return
    threading.Thread(target=measure).start()

def measure():
    global is_measuring, point_counter, current_data, LOG_FILE
    is_measuring = True; samples = []
    print(f"\n[POMIAR] Punkt {point_counter + 1} - Analiza stabilności...")
    ...
    for i in range(AVERAGING_EPOCHS):
        if current_data and current_data['q_txt'] == "FIXED":
            ...
            # Liczymy "pływanie" pozycji na żywo podczas pomiaru
            curr_std = np.std([p['x'] for p in samples]) * 100 if len(samples) > 1 else 0
            print(f"\rPostęp: {i+1}/{AVERAGING_EPOCHS}s | Sigma: {curr_std:.1f} cm", end="")
        else:
    ...
    if len(samples) == AVERAGING_EPOCHS:
        ...
        # Ostateczne odchylenie poziome (2D)
        std_pos = np.sqrt(np.std([p['x'] for p in samples])**2 + np.std([p['y'] for p in samples])**2) * 100
        
        point_counter += 1
        with open(LOG_FILE, 'a', newline='') as f:
            csv.writer(f).writerow([point_counter, time.strftime('%H:%M:%S'), "FIXED", 
                                    round(avg_x, 3), round(avg_y, 3), round(avg_h, 3), 
                                    round(std_pos, 2), round(current_data['pdop'], 2), 
                                    round(current_data['hdop'], 2), round(current_data['vdop'], 2), 
                                    current_data['sv']])
        
        verdict = "IDEALNY" if std_pos < 1.5 else "DOBRY" if std_pos < 4 else "NIEPEWNY (odbicia)"
        print(f"\n[ZAPISANO] Błąd: {std_pos:.1f} cm | Jakosc: {verdict}")
...
LOG_FILE = f"pomiar_v32_{time.strftime('%Y%m%d_%H%M%S')}.csv"

with open(LOG_FILE, 'w', newline='') as f:
    csv.writer(f).writerow(["Nr", "Czas", "Status", "X", "Y", "H", "StdDev_cm", "PDOP", "HDOP", "VDOP", "SV"])
...
print("="*85 + "\n   RTK BOX v32 - MASTER QUALITY MONITOR (P/H/V + SIGMA)\n" + "="*85)
        ...
        if parsed:
            ...
            if parsed.identity in ["GGA", "GNGGA", "GNS", "GNGNS"]:
                if parsed.lat and parsed.lat != 0:
                    ...
                    if not is_measuring:
                        # POWRÓT PEŁNEGO WYŚWIETLANIA
                        print(f"[{time.strftime('%H:%M:%S')}] SV:{sv_sum:02} | P:{dop['p']:.1f} H:{dop['h']:.1f} V:{dop['v']:.1f} | {st_txt:6} | X:{x:.2f} Y:{y:.2f} | H:{h_norm:.3f}m ", end='\r')
