...
# --- KOMENDY RATUNKOWE UBX ---
# 1. Reset do ustawień domyślnych (Cold Start + Config Reset)
UBX_RESET = b'\xb5\x62\x06\x04\x04\x00\xff\xff\x02\x00\x0e\x61'
# 2. Wymuszenie NMEA 4.10 (pozwala na > 12 SV)
CFG_NMEA_41 = b'\xb5\x62\x06\x17\x14\x00\x00\x41\x00\x02\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x75\x57'
# 3. Włącz GGA, GSA, GSV (na wszelki wypadek)
ENABLE_GGA = b'\xb5\x62\x06\x01\x03\x00\xf0\x00\x01\x00\x10'
ENABLE_GSA = b'\xb5\x62\x06\x01\x03\x00\xf0\x02\x01\x02\x16'
ENABLE_GSV = b'\xb5\x62\x06\x01\x03\x00\xf0\x03\x01\x03\x19'
...
def measure():
    ...
    for i in range(AVERAGING_EPOCHS):
        if current_data and current_data['q_txt'] == "FIXED":
            ...
            print("\n[BŁĄD] Przerwano!"); buzzer.beep(0.5, 0.1, 2)
...
print("[*] Resuscytacja modułu u-blox...")
ser.write(UBX_RESET); time.sleep(2) # Czekamy na reboot
ser.write(CFG_NMEA_41); time.sleep(0.2)
ser.write(ENABLE_GGA); time.sleep(0.2)
ser.write(ENABLE_GSA); time.sleep(0.2)
ser.write(ENABLE_GSV); time.sleep(0.2)

geoid = GeoidManager(GTX_FILE); tr = Transformer.from_crs("EPSG:4326", "EPSG:2177", always_xy=True)
LOG_FILE = f"pomiar_v23_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
print("="*70 + "\n   RTK BOX v23 - THE RESURRECTION\n" + "="*70)

try:
    while True:
        (raw, parsed) = nmr.read()
        if parsed:
            # Liczymy satelity z GSV
            if "GSV" in parsed.identity:
                sats_from_gsv = parsed.numSV
            
            # Pozycja z GGA (lub GNS jeśli wskoczy)
            if parsed.identity in ["GGA", "GNGGA", "GNS", "GNGNS"]:
                if parsed.lat and parsed.lat != 0:
                    ...
                    sv_to_show = max(parsed.numSV, sats_from_gsv)
                    ...
                    current_data = {'lat': parsed.lat, 'lon': parsed.lon, 'x': x, 'y': y, 'h': h_norm, 'q_txt': st_txt, 'sv': sv_to_show}
                    
                    if not is_measuring:
                        print(f"[{time.strftime('%H:%M:%S')}] {parsed.identity:5} | SV:{sv_to_show:02} | {st_txt:6} | X:{x:.2f} Y:{y:.2f} | H:{h_norm:.3f}m ", end='\r')
