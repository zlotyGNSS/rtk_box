        ...
        try:
            with socket.create_connection((NTRIP_HOST, NTRIP_PORT), timeout=10) as sock:
                ...
                while True:
                    ...
                    # POPRAWKA: Dodany argument '0' (msgmode) dla pynmeagps
            ...
            print(f"[NTRIP] Błąd: {e}. Reconnect za 5s..."); time.sleep(5)

# --- START ---
geoid = GeoidManager(GTX_FILE); tr = Transformer.from_crs("EPSG:4326", "EPSG:2177", always_xy=True)
ser = serial.Serial(SERIAL_PORT, BAUD, timeout=0.1)

# Wymuszenie NMEA 4.10 (żeby widzieć > 12 SV)
ser.write(b'\xb5\x62\x06\x17\x14\x00\x00\x41\x00\x02\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x75\x57')
time.sleep(0.5)

LOG_FILE = f"pomiar_v18_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
print("="*65 + "\n   RTK BOX v18 - FIX MODE (CORRECTED NTRIP)\n" + "="*65)
    ...
    while True:
        ...
        if parsed and parsed.identity in ["GGA", "GNGGA", "GNS", "GNGNS"]:
            if parsed.lat and parsed.lat != 0:
                ...
                current_data = {'lat': parsed.lat, 'lon': parsed.lon, 'x': x, 'y': y, 'h': h_norm, 'q_txt': st_txt}
