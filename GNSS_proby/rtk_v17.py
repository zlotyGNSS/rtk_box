...
# --- KOMENDY UBX (Inicjalizacja) ---
# Przełączenie na NMEA 4.10 i włączenie GNS
...
ENABLE_GNS = b'\xb5\x62\x06\x01\x03\x00\xf0\x0d\x01\x02\x34' # Włącz wiadomość GNS
...
class GeoidManager:
    def __init__(self, path):
        ...
        except Exception as e: print(f"Błąd geoidy: {e}"); self.interp = None
...
def handle_button():
    ...
    if current_data['q_txt'] != "FIXED":
        print(f"\n[BLOKADA] Brak FIXED! Status: {current_data['q_txt']}")
    ...
    while True:
        try:
            with socket.create_connection((NTRIP_HOST, NTRIP_PORT), timeout=10) as sock:
                ...
                print("[NTRIP] Połączono z bazą, wysyłam poprawki...")
                while True:
                    # Wysyłaj GGA do bazy, by wiedziała gdzie jesteś (VRS)
                    l, n = (current_data['lat'], current_data['lon']) if current_data else (52.85, 17.71)
                    now = time.strftime("%H%M%S", time.gmtime())
                    gga = pynmeagps.NMEAMessage("GP", "GGA", time=now, lat=l, NS="N", lon=n, EW="E", quality=1, numSV=12, HDOP=1.0, alt=100.0, altUnit="M", sep=33.0, sepUnit="M")
                    ...
                    if d: 
                        ser.write(d)
                        # Opcjonalne pikanie LED przy poprawkach? Lepiej nie, będzie migać jak szalone
        except Exception as e:
            print(f"[NTRIP] Błąd: {e}. Reconnect za 5s...")
            time.sleep(5)
...
time.sleep(0.2)
ser.write(ENABLE_GNS)
time.sleep(0.2)

LOG_FILE = f"pomiar_v17_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
print("="*65 + "\n   RTK BOX v17 - DEEP DIAGNOSTIC (GNS MODE)\n" + "="*65)

try:
    while True:
        (raw, parsed) = nmr.read()
        # Obsługujemy GGA ORAZ GNS (GNS jest lepsze dla wielu konstelacji)
        if parsed and parsed.identity in ["GGA", "GNGGA", "GNS", "GNGNS"]:
            if parsed.lat and parsed.lat != 0:
                # W GNS pole quality to ciąg znaków (np. 'RR'), w GGA to int
                q = 0
                if parsed.identity in ["GNS", "GNGNS"]:
                    pos_mode = parsed.posMode
                    # R = RTK Fixed, F = RTK Float, A = Autonomous
                    q = 4 if 'R' in pos_mode else 5 if 'F' in pos_mode else 1
                else:
                    q = parsed.quality
                ...
                current_data = {'lat': parsed.lat, 'lon': parsed.lon, 'x': x, 'y': y, 'h': h_norm, 'q': q, 'q_txt': st_txt}
                
                if not is_measuring:
                    print(f"[{time.strftime('%H:%M:%S')}] {parsed.identity:5} | SV:{parsed.numSV:02} | {st_txt:6} | X:{x:.2f} Y:{y:.2f} | H:{h_norm:.3f}m ", end='\r')
