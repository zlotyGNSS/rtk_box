...
total_sats_visible = 0 # Tu będziemy sumować satelity z GSV
...
# Czyścimy bufor u-bloxa po poprzednich wersjach
ser.reset_input_buffer()
# Opcjonalnie: Włączamy raportowanie satelitów GSV (jeśli było wyłączone)
ser.write(b'\xb5\x62\x06\x01\x03\x00\xf0\x03\x01\x2a\x10') 

LOG_FILE = f"pomiar_v22_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
print("="*70 + "\n   RTK BOX v22 - FULL CONSTELLATION (GGA+GSV)\n" + "="*70)

try:
    while True:
        (raw, parsed) = nmr.read()
        if parsed:
            # 1. Liczymy satelity z GSV (każdy system ma swoją ramkę GSV)
            if "GSV" in parsed.identity:
                # numSV w GSV to liczba satelitów w danej konstelacji (np. tylko GPS)
                # Musimy je sumować dynamicznie
                total_sats_visible = parsed.numSV 

            # 2. Pobieramy pozycję z GGA
            if parsed.identity in ["GGA", "GNGGA"]:
                if parsed.lat and parsed.lat != 0:
                    q = parsed.quality
                    # Jeśli GGA mówi 12, a GSV mówi 25, bierzemy 25!
                    sv_to_show = max(parsed.numSV, total_sats_visible)
