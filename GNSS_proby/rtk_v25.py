...
sats_visible = {} # Słownik na satelity z różnych konstelacji
...
ser.write(b'\xb5\x62\x06\x01\x03\x00\xf0\x0d\x01\x02\x34')

geoid = GeoidManager(GTX_FILE); tr = Transformer.from_crs("EPSG:4326", "EPSG:2177", always_xy=True)
LOG_FILE = f"pomiar_v25_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
print("="*70 + "\n   RTK BOX v25 - THE FINAL COUNTDOWN\n" + "="*70)

try:
    while True:
        (raw, parsed) = nmr.read()
        if parsed:
            # Zbieramy satelity z ramek GSV (każda konstelacja wysyła swoją)
            ...
            if parsed.identity in ["GGA", "GNGGA", "GNS", "GNGNS"]:
                if parsed.lat and parsed.lat != 0:
                    ...
                    # Sumujemy satelity ze słownika (prawdziwa liczba!)
