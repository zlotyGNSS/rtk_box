...
sats_visible = {} # Słownik do przechowywania liczby SV z różnych systemów
...
def handle_button():
    ...
    if current_data['q_txt'] != "FIXED":
        print(f"\n[INFO] Brak statusu FIXED. Czekaj..."); buzzer.beep(0.3, 0.1, 1); return
...
LOG_FILE = f"pomiar_v27_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
print("="*70 + "\n   RTK BOX v27 - SMART PASSIVE (NO HANGS, FULL SV)\n" + "="*70)

try:
    while True:
        (raw, parsed) = nmr.read()
        if parsed:
            # KLUCZ: Sumujemy satelity z ramek GSV, które u-blox wysyła sam z siebie
            if "GSV" in parsed.identity:
                # Identyfikujemy konstelację po nagłówku (GP, GL, GA, GB)
            ...
            if parsed.identity in ["GGA", "GNGGA", "GNS", "GNGNS"]:
                if parsed.lat and parsed.lat != 0:
                    ...
                    current_data = {'lat': parsed.lat, 'lon': parsed.lon, 'x': x, 'y': y, 'h': h_norm, 'q_txt': st_txt, 'sv': real_sv}
                    
                    if not is_measuring:
                        print(f"[{time.strftime('%H:%M:%S')}] SV:{real_sv:02} | {st_txt:6} | X:{x:.2f} Y:{y:.2f} | H:{h_norm:.3f}m ", end='\r')
